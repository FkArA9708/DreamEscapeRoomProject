...
(2, 'Three statements appear on the wall. Only one is true. Which one will lead you to wake up?', 'In a dream, you are dreaming — so that means statement B is false.\r\nNow, if C is true, that would make both A and B false — that fits.\r\nBut test all options: only one statement can be true.', 'The mirror holds the code', 1),
